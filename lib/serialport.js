...
/**
 * Open event.
 * @event SerialPort#open
 * @description Callback is called with no arguments when the port is opened and ready for writing. This happens if you have the constructor open immediately (which opens in the next tick) or if you open the port manually with `open()`. See [Useage/Open Event](#open-event) for more information.
 */
 ...
 * @param {errorCallback} [callback] Called once the port's baud rate has been changed. If `.update` is called without an callback and there is an error, an error event will be emitted.
 */
 ...
 * @param  {Function} callback Called once the write operation returns.
 ...
 * @param  {Function=} callback Called once the flush operation finishes.
 */
 ...
 * @param {Function=} callback Called once the port's flags have been set.
 */
 ...
 * @param {Function} callback Called once the drain operation returns.
 * @example
